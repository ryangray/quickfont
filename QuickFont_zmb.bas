...
# TODO:
#
# - [x] Make the UI BG color cycle around after white back to black
# - [x] Merge in the mods for capturing the loaded file name
# - [x] Merge in the mods for UDG edit/save/load bytes
# - [x] Need to keep f$ for font name and u$ for UDG name
# - [x] Add show bytes feature (sh+1)
# - [ ] Add option to print the byte values they can show up in printout.txt.
# - [x] Change copy rom char to e/E/>= in edit mode
# - [x] Add extra keys help on ? key in edit mode
# - [x] Add h/v flip (not MC)
# - [x] Add l/r rotate (not MC)
# - [x] Improve menu layout
# - [x] Use more overprinting to eliminate need for blank menu area routine
#
...
# We move the call to decode down into a copy of 82 so we only switch fonts once
   ...
   77 RETURN 
#
# I suspect 0,60 also works for OS-64, but OS-64 copies to 0,xxx and sets FONT to that
...
#
  ...
  459 RETURN 
#
# Could move Quit to near end of program
...
#
  ...
  680 RETURN 
#
# LINE  700 range empty
# LINE  800 range empty
# LINE  900 range empty
# LINE 1000 range empty
#
  900 REM Cancel
...
#
# LINE 1000 range empty
# LINE 2000 range empty
# LINE 3000 range empty
#
 4000 REM Test
...
#
 ...
 6120 LET z$=z$+k$
# The 48K Spectrum emulation in Fuse wouldn't work right with the original code:
#   6120 LET z$=z$+SCREEN$ (b,6+a)
# This would cause z$ to be overwritten with two of the SCREEN$ character 
# rather than being appended to. Need to see if that happens on a real Spectrum.
 6130 NEXT a
 6140 LET i=LEN z$
...
#
# NOTE: maybe move this up to speed it up?
...
#
 ...
 7560 RETURN 
#
# NOTE: maybe move these up to speed them up?
...
#
# Had problems with rotation ASM
# For OS-64, the ROM font addr needs to be poked into the REM
# ** I think the ROM font in OS-64 is still at 0,60, so fine for copying from. *Check it*
# For Spectrum, then UDG addr needs to be poked into the REM
#
